...
4. **Market Data**: Controls whether trading runs live against MT5 or replays stored environment snapshots
...
The `market_data` section toggles between live MT5 execution and offline snapshot replay:

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `mode` | string | `"live"` | Set to `"live"` for real MT5 trading or `"snapshot"` to replay stored market environments |
| `snapshot_path` | string | `"data/env_data.pkl"` | Pickle generated by the snapshot builder (default `data/env_data.pkl`) |
| `llm_executor` | string/obj | _optional_ | Identifier or callable passed to the `EnvironmentAgent` when running snapshots |
| `llm_config` | object | _optional_ | Extra keyword arguments forwarded to the built-in LLM backend |

> **Tip:** `snapshot_path` can be absolute or relative to the project root. Create the directory before running snapshot mode.
...
### Mode 3: Snapshot Replay (Environment Agent)

**Behavior:**
1. Loads a pickled dataset of daily snapshots (price, news, filings)
2. Instantiates `MarketEnvironment` and `EnvironmentAgent`
3. Steps through each snapshot sequentially, combining strategy signals and optional LLM reasoning
4. Applies risk checks and prints simulated trade actions (no live MT5 orders are placed)

**How to build a snapshot file:**
```bash
python scripts/build_env_data.py --symbol EURUSD --start 2024-01-01 --end 2024-02-01
```

- The command writes `data/env_data.pkl` by default.
- Adjust `--start`/`--end` to control the replay window.
- The script reads optional date defaults from `config/settings.json` under `environment_data`.

**Configuration:**
```json
"market_data": {
  "mode": "snapshot",
  "snapshot_path": "data/env_data.pkl"
}
```

Optionally include `llm_executor`/`llm_config` to enable an LLM backend during replay.

**Best for:**
- Offline evaluation of strategy combinations
- Prompt engineering for the environment agent
- Generating reproducible transcripts for analytics
